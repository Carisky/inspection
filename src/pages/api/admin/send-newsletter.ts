...
export default async function handler(
  ...
  // Получаем выбранный шаблон из базы данных
  ...
  // Настройка транспортера nodemailer (не забудьте настроить переменные окружения)
  const transporter = nodemailer.createTransport({
    host: process.env.SMTP_HOST, // например, smtp.gmail.com
    port: parseInt(process.env.SMTP_PORT || '587'),
    secure: process.env.SMTP_PORT === '465', // true для 465, иначе false
    auth: {
  ...
  try {
    const info = await transporter.sendMail({
      from: process.env.SMTP_FROM, // например, 'no-reply@yourdomain.com'
      to: recipient,
      subject,
      html,
    });
    ...
    return res
      .status(500)
      .json({ error: 'Ошибка отправки: ' + err.message });
  }
}
